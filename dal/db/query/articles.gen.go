...
func newArticle(db *gorm.DB, opts ...gen.DOOption) article {
	...
	_article.Label = field.NewString(tableName, "label")
...
type IArticleDo interface {
	...
	FilterWithNameAndRole(name string, role string) (result []model.Article, err error)
}

// FilterWithNameAndRole SELECT * FROM @@table WHERE name = @name{{if role !=""}} AND role = @role{{end}}
func (a articleDo) FilterWithNameAndRole(name string, role string) (result []model.Article, err error) {
	var params []interface{}

	var generateSQL strings.Builder
	params = append(params, name)
	generateSQL.WriteString("SELECT * FROM articles WHERE name = ? ")
	if role != "" {
